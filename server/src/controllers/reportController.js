...
const dispatchReport = async (req, res) => {
  try {
    ...
    const fileName = `Garud-${department.code}-${district || 'Gujarat'}-${timestampStr}.${fileExtension}`;
    ...
    if (sendEmail) {
      try {
        const mailResult = await sendReportEmail({
          to: recipient,
          subject: `[Garud Official Audit] ${reportTitle} - ${new Date().toLocaleDateString()}`,
          html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #1e293b; line-height: 1.6;">
              <div style="background-color: #0f172a; padding: 20px; border-radius: 8px 8px 0 0; color: #38bdf8;">
                <h2 style="margin: 0; font-size: 18px;">GARUD GUJARAT SURVEILLANCE</h2>
              ...
              <div style="padding: 24px; border: 1px solid #e2e8f0; border-top: none; background: #ffffff;">
                ...
                <p>Please find attached the official analytical audit report generated by the Garud Command Engine for <strong>${department.name}</strong>.</p>
