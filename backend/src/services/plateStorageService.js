/**
 * Dedicated Plate Storage Service for DrishtiGrid ANPR.
 * 
 * Manages persistent storage of detected license plates:
 * 1. MongoDB Atlas 'storedplates' collection (with automatic fallback)
 * 2. Flat-file JSON registry: `backend/data/stored_number_plates.json`
 * 3. Plain-text clean string list: `backend/data/stored_number_plates.txt`
 */
...
/**
 * Read stored plates from local JSON file
 */
...
/**
 * Write stored plates to local JSON file
 */
...
/**
 * Re-generate the clean plain-text string list file (stored_number_plates.txt)
 */
...
async function storePlate(plateData) {
  ...
  // 1. Try to save to MongoDB Atlas 'storedplates' collection
  let mongoDoc = null;
  if (isMongoConnected()) {
    try {
 ...
 * @returns {Array<string>} list of strings e.g. ["DL8CAF5030", "BZ9811053", ...]
 */
